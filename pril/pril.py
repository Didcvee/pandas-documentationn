...
# Функция для получения списка станций из базы данных ClickHouse
def get_station_list():
    client = Client('localhost')  # Замените 'localhost' на ваш хост ClickHouse
    query = "SELECT DISTINCT Station FROM metro.station ORDER BY Station"
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("Cluster Station Load"):
        with gr.Row():
            ...
            station = gr.Dropdown(choices=station_list, label="Station")
    ...
    gr.Markdown("""
    ### User Guide
    - **Cluster Station Load**: Select a cluster, choose a station, and choose a period to view the load forecast.
